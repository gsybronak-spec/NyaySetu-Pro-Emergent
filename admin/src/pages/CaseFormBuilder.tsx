...
export default function CaseFormBuilder() {
  ...
  const addField = () => {
    ...
    setConfig({ ...config, fields: [...currentFields, newField] });
  };
  ...
  const moveField = (index: number, direction: 'up' | 'down') => {
    ...
    setConfig({ ...config, fields: currentFields });
  };
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1000px', margin: '0 auto' }}>
      ...
      {loading || !config ? (
        <div style={{ padding: '24px', textAlign: 'center', color: '#64748b' }}>Loading case form configuration...</div>
      ) : (
        <div style={{ backgroundColor: '#ffffff', borderRadius: '8px', border: '1px solid #e2e8f0', padding: '20px' }}>
          ...
          {(config.fields || []).length === 0 ? (
            <div style={{ textAlign: 'center', padding: '36px 16px', color: '#64748b', backgroundColor: '#f8fafc', borderRadius: '6px', border: '1px dashed #cbd5e1' }}>
              <p style={{ margin: '0 0 6px 0', fontWeight: 600, color: '#334155', fontSize: '14px' }}>No custom fields configured for this case type.</p>
              <p style={{ margin: 0, fontSize: '13px' }}>Standard client information (name, mobile, address, district) is captured automatically by the Client Details section. Click <strong>+ Add Custom Field</strong> above to configure case-specific fields.</p>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {(config.fields || []).map((field, idx) => (
                <div key={idx} style={{ padding: '12px', border: '1px solid #e2e8f0', borderRadius: '6px', backgroundColor: '#f8fafc', display: 'grid', gridTemplateColumns: '40px 1fr 1.5fr 1.5fr 1fr 1.5fr 60px 40px', gap: '10px', alignItems: 'center' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', alignItems: 'center' }}>
                    <button
                      type="button"
                      disabled={idx === 0}
                      onClick={() => moveField(idx, 'up')}
                      style={{ padding: '1px 4px', fontSize: '10px', cursor: idx === 0 ? 'default' : 'pointer', opacity: idx === 0 ? 0.3 : 1, border: '1px solid #cbd5e1', borderRadius: '3px', background: '#fff' }}
                      title="Move Up"
                    >
                      ▲
                    </button>
                    ...
                    <button
                      ...
                      style={{ padding: '1px 4px', fontSize: '10px', cursor: idx === (config.fields || []).length - 1 ? 'default' : 'pointer', opacity: idx === (config.fields || []).length - 1 ? 0.3 : 1, border: '1px solid #cbd5e1', borderRadius: '3px', background: '#fff' }}
                      title="Move Down"
                    >
                      ▼
                    </button>
